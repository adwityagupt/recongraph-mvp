...
def main():
    ...
    if run and left_file and right_file:
        df_left = load_table(left_file, "csv" if left_file.name.lower().endswith(".csv") else "xlsx")
        df_right = load_table(right_file, "csv" if right_file.name.lower().endswith(".csv") else "xlsx")
        ...
        st.dataframe(exL.head(100), use_container_width=True)

        st.subheader("Unmatched — Right")
        st.dataframe(exR.head(100), use_container_width=True)

    # 3) Certificate / Report generation
    st.markdown("---")
    ...
    if st.button("📄 Generate"):
        if matches is None:
            st.warning("Please run reconciliation first (upload files and click 'Run Reconciliation').")
        else:
